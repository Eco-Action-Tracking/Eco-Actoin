...
const EnvironmentalImpact = () => {
  ...
  const impactMetrics = [
    { icon: <Leaf className="w-12 h-12 text-indigo-600" />, label: 'kg of CO2 emissions saved', value: calculateImpact(7) },
    { icon: <Droplet className="w-12 h-12 text-blue-500" />, label: 'liters of water saved', value: calculateImpact(2700) },
    { icon: <Wind className="w-12 h-12 text-purple-500" />, label: 'kWh of energy saved', value: calculateImpact(11) },
  ];
    ...
    <section className="py-20 bg-gradient-to-br from-indigo-50 to-blue-50">
      <div className="container mx-auto px-4">
        <motion.div
          ...
          className="text-center mb-16"
        >
          <h2 className="text-5xl font-bold mb-4 text-indigo-600">Recycling Helps Our Planet</h2>
        ...
        <div className="mb-12">
          <div className="flex justify-center space-x-4 mb-8">
            <button
              onClick={() => setActiveTab('chart')}
              className={`px-6 py-2 rounded-full ${activeTab === 'chart' ? 'bg-indigo-600 text-white' : 'bg-gray-200'}`}
            >
              Comparison Chart
            </button>
            <button
              onClick={() => setActiveTab('calculator')}
              className={`px-6 py-2 rounded-full ${activeTab === 'calculator' ? 'bg-indigo-600 text-white' : 'bg-gray-200'}`}
            >
              Impact Calculator
            </button>
          </div>
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'chart' && (
              <motion.div
                ...
                transition={{ duration: 0.5 }}
              >
                <h3 className="text-3xl font-semibold mb-6 text-center text-indigo-600">Environmental Impact Comparison</h3>
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={environmentalData} layout="vertical">
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis type="number" />
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="standard" name="Regular Clothes" fill="#4f46e5" /> {/* Changed to indigo color */}
            ...
            {activeTab === 'calculator' && (
              <motion.div
                ...
                className="bg-white p-8 rounded-xl shadow-2xl"
              >
                <h3 className="text-3xl font-semibold mb-6 text-center text-indigo-600">Your Environmental Impact Calculator</h3>
                <div className="flex flex-col items-center mb-8">
                  ...
                  <div className="relative w-full max-w-md">
                    ...
                    />
                    <div 
                      className="absolute top-0 left-0 h-3 bg-indigo-600 rounded-full pointer-events-none" 
                      style={{ width: `${recycledClothes}%` }}
                    ></div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {impactMetrics.map((metric, index) => (
                    <motion.div 
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.5, delay: index * 0.1 }}
                      className="bg-gradient-to-br from-indigo-100 to-blue-100 p-6 rounded-lg shadow-lg text-center"
                    >
                      <div className="flex justify-center mb-4">{metric.icon}</div>
                      <motion.h4 
                        className="text-3xl font-bold mb-2"
        ...
        >
          ...
          <Recycle className="w-16 h-16 text-indigo-600 mx-auto animate-spin-slow" />
        </motion.div>
      </div>
    </section>
  );
